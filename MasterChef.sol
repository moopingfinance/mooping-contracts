...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        address nftAddress; // Address of the NFT which is required to use the pool. (Optional)
    }
    ...
    function set(
        ...
        poolInfo[_pid].nftAddress = _nftAddress;
    }
        ...
        returns (bool)
    {
        PoolInfo storage pool = poolInfo[_pid];
        return
            pool.nftAddress == address(0) ||
            IERC721(pool.nftAddress).balanceOf(_user) >= 1;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(
            pool.nftAddress == address(0) ||
                IERC721(pool.nftAddress).balanceOf(msg.sender) >= 1,
